...
from explore import (
    ...
    retrieve_query_data,
    retrieve_aqp_data,
    retrieve_content_for_block_no,
    AnnotatorHelper,
    retrieve_buffer_access_data,
    retrieve_block_access_count,
...
class Main:
    ...
    def login(self):
        ...
        login_ui = Login(
            self.login_details
        )  # Pass login details into Login class instance
        ...
        return (
            login_ui.login_details
        )  # Returns the inputted login details to be updated in our Main class
    ...
    def get_qep_from_query(self, database, query):
        ...
        qep = retrieve_query_data(
            self.login_details, query_details
        )  # Gets output from EXPLAIN function
        if qep is None:
            return "Query is not valid. Try again.", -1
        buffer_data = retrieve_buffer_access_data(self.login_details, query_details)
        qep_with_details = AnnotatorHelper().procedure_string(
            qep, buffer_data
        )  # Helper function to get natural langauge of qep
        return (
            str(qep_with_details),
            qep[0][0][0]["Plan"]["Total Cost"],
        )  # Return natural lang + total cost info
    ...
    def get_aqp(self, perm_list, database, query):
        ...
        aqp = retrieve_aqp_data(
            self.login_details, querydetails, perm_list
        )  # Passes in details and parameters list and get AQP Plan
    ...
    def get_content_in_specified_block(self, database, query, block):
        ...
        block_content = retrieve_content_for_block_no(
            self.login_details, querydetails, block
        )
